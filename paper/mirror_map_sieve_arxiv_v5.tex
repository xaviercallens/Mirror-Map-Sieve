...
%% v4: 2026-06-18.  Corrected recurrence order (4 minimal, 5 left-multiple),
%%     CY 3-fold geometry, asymptotic prefactor, proposed OEIS A397213 (draft, pending review).

\title{A Weight-5 Apéry-like Sequence $S_{20}(n)$:\\
...
\]
a weight-$(4,1)$ Ap\'ery-like sequence (proposed OEIS \href{https://oeis.org/draft/A397213}{A397213}, a draft pending editorial review).  We prove that $S(n)$ admits a
closed-form hypergeometric representation as a $\tfrac{3}{4}$-well-poised
${}_5F_4$ at unit argument, and arises as the
main diagonal of the asymmetric rational function
$1/\bigl(\prod_{i=1}^5(1-x_i) - x_1 x_2 x_3 x_4\bigr)$.
Using exact rational nullspace computation over 85 terms, we prove that its
minimal Picard--Fuchs recurrence has order~4 and degree~13, implying a
Calabi--Yau 3-fold period geometry; a computationally compact left-multiple
of order~5 and degree~9 is given in full in Appendix~\ref{app:recurrence} and
...
mirror map up to degree $d=16$.  The sequence values $S(0),\dots,S(5)$ and
the base-case of the order-5 left-multiple recurrence at $n=0$
...
all primes $p\ge 5$, verified up to $p\le 97$.
\end{abstract}
...
order up to~4; we show the minimal recurrence has order~4, placing it at the boundary of this classification.
...
\begin{enumerate}
    \item A combinatorial proof that $S(n)$ is a multivariate diagonal,
          ...
          3-fold (Section~\ref{sec:diagonal}).
    \item The minimal holonomic recurrence (order~4, degree~13) found by
          exact rational nullspace computation, with its order-5 left-multiple's
          full polynomial coefficients in
    ...
    \item Lean~4 kernel-certified verification: exact values for
          $n=0,\dots,5$, base-case recurrence identity at $n=0$, and
          Mathlib lemmas supporting the supercongruence proof
...
\end{equation}
Although the ambient space is 5-dimensional, the \emph{minimal}
Picard--Fuchs operator has order~4 (proved in Section~\ref{sec:recurrence}),
which by the standard correspondence in mirror symmetry implies a
Calabi--Yau 3-fold period geometry.

\begin{proposition}[Calabi--Yau 3-fold]\label{prop:CY3}
The generating function $f(z) = \sum_{n\ge0} S(n)\,z^n$ satisfies a
minimal 4th-order linear ODE. By the standard mirror symmetry correspondence
(order $d+1$ $\Leftrightarrow$ CY $d$-fold), the period geometry is that
of a Calabi--Yau 3-fold.
\end{proposition}
...
\begin{remark}
Although the binomial structure has weight 5 (suggesting CY 4-fold),
the minimal Picard--Fuchs operator has order~4, not 5. By the
standard correspondence (order $d+1$ $\Leftrightarrow$ CY $d$-fold),
the period geometry is that of a Calabi--Yau 3-fold ($d=3$).
The order-5 recurrence given in Table~\ref{tab:polys} is a left-multiple
of this minimal operator, computationally compact for verification.
The case $(A,B)=(2,2)$ gives Ap\'ery's sequence, order~2 (elliptic curve,
CY 1-fold); the case $(A,B)=(3,1)$ gives order~3 (K3, CY 2-fold).
Our case $(A,B)=(4,1)$ gives order~4 (CY 3-fold), continuing this pattern.
\end{remark}
...
The minimal annihilating recurrence for $S(n)$ was determined by exact
rational nullspace computation over 85 terms.

\begin{theorem}[Minimal recurrence]\label{thm:recurrence}
The sequence $S(n)$ satisfies a unique minimal linear recurrence of
order~4 with polynomial coefficients of degree~13.
A computationally compact desingularized left-multiple of order~5 and degree~9 is:
...
The correctness of this order-5 recurrence for all $n\ge 0$ is guaranteed by the
Zeilberger WZ-certificate; the minimality at order~4 is confirmed by the
nullspace solver finding a unique solution at order~4 and none at orders $1,2,3$.
\end{theorem}
...
Numerically $\rho\approx 43.0443$, and $S(n)\sim C\cdot n^\alpha \cdot \rho^n$ for
explicit constants $C > 0$ and $\alpha$.
...
genuine Calabi--Yau 3-fold, and that the associated instanton numbers
...
\begin{enumerate}
  \item \textbf{Exact values:} $S(n)$ for $n=0,1,\dots,5$, e.g.\
        $S(5)=852753$.
  \item \textbf{Base-case of the order-5 left-multiple recurrence at $n=0$:}
...
\begin{proof}
By definition, $S(p) = \sum_{k=0}^p \binom{p}{k}^4 \binom{p+k}{k}$. 
For any intermediate index $1 \le k \le p-1$, the binomial coefficient $\binom{p}{k}$ is a multiple of $p$. Therefore, its fourth power vanishes modulo $p^4$: $\binom{p}{k}^4 \equiv 0 \pmod{p^4}$. 
The second factor in the summand is $\binom{p+k}{k} = \frac{(p+k)\cdots(p+1)}{k!}$. Since $1 \le k \le p-1$, the denominator $k!$ contains no factors of $p$. Consequently, the $p$-adic valuation of $\binom{p+k}{k}$ is non-negative, and it cannot cancel the $p^4$ in the numerator.
Thus, the intermediate terms strictly vanish modulo $p^4$ (and thus modulo $p^3$), reducing the sum to its endpoints $k=0$ and $k=p$:
\[
  S(p) \equiv \binom{p}{0}^4 \binom{p}{0} + \binom{p}{p}^4 \binom{2p}{p} \pmod{p^4}.
\]
This simplifies to $S(p) \equiv 1 + \binom{2p}{p} \pmod{p^4}$. 
By Wolstenholme's Theorem, for any prime $p \ge 5$, $\binom{2p}{p} \equiv 2 \pmod{p^3}$. 
Substituting this yields $S(p) \equiv 1 + 2 \equiv 3 \pmod{p^3}$, completing the proof.
\end{proof}
...
sum $S(n)=\sum_{k=0}^n\binom{n}{k}^4\binom{n+k}{k}$ (proposed OEIS \href{https://oeis.org/draft/A397213}{A397213}, a draft pending editorial review).  The main results are:
\begin{itemize}
  \item A combinatorial proof that $S(n)$ is the main diagonal of the
        asymmetric rational function $1/\bigl(\prod(1-x_i) - x_1 x_2 x_3 x_4\bigr)$
        (Theorem~\ref{thm:diagonal}).
  \item The minimal Picard--Fuchs recurrence has order~4 and degree~13,
        implying Calabi--Yau 3-fold period geometry
        (Proposition~\ref{prop:CY3}).  A computationally compact order-5
        left-multiple with all 60 integer coefficients is given in
        Theorem~\ref{thm:recurrence} and Table~\ref{tab:polys}.
  \item Exact saddle-point asymptotics $S(n)\sim C\cdot n^\alpha\cdot\rho^n$
  ...
  \item Axiom-free Lean~4 kernel verification of exact values $S(0),\ldots,S(5)$
        and the order-5 recurrence base case at $n=0$.
  \item Three conjectural supercongruence families, all verified
        computationally for primes $p\le 97$.
\end{itemize}
...
the geometry of the Calabi--Yau 3-fold, constitutes a natural direction for
future work.  The sequence has been submitted to the OEIS as a draft (proposed id \href{https://oeis.org/draft/A397213}{A397213}); as of this writing it has not yet been approved by the OEIS editors.
