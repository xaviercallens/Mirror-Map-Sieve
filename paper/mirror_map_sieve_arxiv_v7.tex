...
\title{A Weight-5 Ap\'ery-like Sequence $S_{20}(n)$:\\
  ...
  \large (preprint, v7 --- under community review)}
...
%% =====================================================================
\appendix
\section{An applied side-quest, reported in full: a learnable positional
bias inspired by---but not explained by---the Calabi--Yau structure}
\label{app:attention}
%% =====================================================================

We record here an engineering experiment that the holonomic structure of $\Sx$
\emph{inspired}, together with its honest---and partly cautionary---outcome. It
is not part of the mathematical contribution; we include it because the negative
and null results are as informative as the positive one, and because reporting
them is the discipline this preprint tries to model.

\paragraph{Hypothesis.} Accelerators are memory-bandwidth bound. A positional
attention bias generated \emph{on the fly} from the proved order-$4$ recurrence
of $\Sx$ would cost $\mathcal{O}(L)$ memory rather than an $\mathcal{O}(L^2)$
table---\emph{if} it preserves model quality. We fixed, in advance, a quality
gate: PASS within $+1\%$ of the best baseline perplexity, KILL at $>5\%$
regression (all on WikiText-2, models trained from scratch, one per positional
scheme, at matched compute---the ALiBi-paper methodology).

\paragraph{Outcome, in order.}
\begin{enumerate}\itemsep2pt
\item A Triton kernel reproduces the reference attention to FP16 tolerance
  (correctness gate PASS).
\item The \emph{fixed} bias $-\log\Sx(d)$ \textbf{fails} the quality gate
  ($+10.2\%$ perplexity; a plain sliding window wins). Its slope
  $\log\lambda\approx 3.76$ is too steep---a brick wall.
\item Making the per-head slope \emph{learnable},
  $\mathrm{bias}_h(d)=-\gamma_h\log\Sx(d)$ with $\gamma_h$ trained, \textbf{passes
  at scale}: $+8.1\%$ better perplexity than the best baseline ($3.32$ vs
  $3.62$), with stable length-extrapolation where learned absolute positions
  collapse. (An intermediate run taught a methodological lesson: a short
  training budget crowned the high-capacity bias, which then overfit at full
  budget; $\gamma$-regularization and validation early-stopping resolve it.)
\item \textbf{Attribution.} A nested bias $\mathrm{bias}_h(d)=-a_h\,d+b_h\log d$
  (slope $a$ and log-curvature $b$ both learnable per head) shows the gain comes
  from \emph{learnability}, not the Calabi--Yau shape: a learnable ALiBi slope
  with $b\equiv0$ already beats the fixed holonomic bias, and when $b$ is free
  the optimizer drives it \emph{away} from the geometric value $\beta=2$. We
  therefore drop the $\Sx$/recurrence machinery and report the result as what it
  is---a learnable linear-plus-log positional bias---rather than as a
  number-theoretic method.
\item \textbf{Final form selected by elimination.} Three independent GPU bake-offs
  converge on the simplest scheme. A six-family comparison (linear, log-curvature,
  Fourier, fixed Calabi--Yau, local+tail hybrid, exponential decay) leaves
  \emph{learnable per-head ALiBi} the winner; everything more expressive loses at
  scale. A final orthogonal probe of per-head content/position balance---a learnable
  scale $c_h$ on the content score, optionally with a per-head softmax
  temperature---only \emph{ties} learnable-ALiBi ($+0.35\%$ at $4\times$ the
  training length), while the temperature knob \emph{regresses}. The shipped
  hypothesis is thus one learnable linear slope per head and nothing else: a single
  per-head scalar, $\mathcal{O}(1)$ state, no curvature, no sequence, no content
  scale. \emph{Adaptability beats added structure---up to that one slope.}
\end{enumerate}

\paragraph{What we claim, and what we do not.} We claim a small, reproducible
quality result (a learnable per-head positional bias competitive with, and on
this benchmark a few percent better than, ALiBi, with good extrapolation). We do
\emph{not} claim that the Calabi--Yau geometry improves attention (the
attribution refutes it), nor a memory advantage over FlashAttention$+$ALiBi
(which already generates its bias on the fly; a learnable per-head scalar is
$\mathcal{O}(1)$ state, identical to ALiBi). The $\mathcal{O}(L)$-vs-$\mathcal
{O}(L^2)$ saving holds only against a \emph{materialized} bias table, a strawman
in modern kernels. Full data and code are in the repository
(\texttt{docs/CYSIEVE\_JOURNEY.md}) and an open benchmark dataset; we would value
a fair comparison against learnable-ALiBi, NoPE, and RoPE-scaling on a genuine
long-range task.
