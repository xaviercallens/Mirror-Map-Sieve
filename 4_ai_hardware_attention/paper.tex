...
Context extension for transformer models has emerged as a key area of study. Early attempts at training-free extrapolation led to the development of Attention with Linear Biases (ALiBi) by Press et al. \cite{press2022alibi}, which forms the architectural basis of our work. For Rotary Position Embedding (RoPE) networks, interpolation methods such as YaRN \cite{yarn2023} and NTK-Aware scaling \cite{su2023ntk} have achieved significant success in extending sequence length, although they require additional pretraining or tuning steps.

In parallel, parameter-efficient fine-tuning (PEFT) has been widely adopted to mitigate the memory footprint of training. Low-Rank Adaptation (LoRA) \cite{lora2021} and its variants like Weight-Decomposed LoRA (DoRA) \cite{dora2024} and AdaLoRA \cite{adalora2023} optimize low-rank updates to projection weights. However, as demonstrated in our work, joint optimization of projection adapters and positional parameters (slopes) can trigger representational mismatches during validation early-stopping. This paper proposes a minimal-interference, slopes-only parameterization that achieves superior performance and hardware efficiency.
...
\textit{Rationale for active-slope regularization:} Rather than regularizing log-slopes ($\gamma \sum \theta_h^2$), which would contract log-slopes toward zero and force active slopes to $a_h \to 1$ (yielding an overly steep positional decay), we apply L2 regularization directly to the physical active slopes $a_h$. This encourages a gentle contraction toward zero without forcing mathematical collapse, preventing both extreme steepness and flat attention profiles. In our sweeps, we set $\gamma = 0.01$ as a soft stability constraint.

\textit{Practical enforcement of slope bounds:} To guarantee that $a_h \ge \alpha_{\text{min}} > 0$ is strictly satisfied in practical implementations (preventing zero-slope division or local attention collapse), we clip the parameterized log-slopes during continued pretraining:
\begin{equation}
    \theta_{h} \ge \log(\alpha_{\text{min}}) \quad \forall h \in \{1, \dots, H\}
\end{equation}
where we set $\alpha_{\text{min}} = 10^{-4}$ as our absolute lower slope threshold.
...
--/
def AlibiAttentionWeight (a : Real) (i j : Nat) (s : Real) : Real :=
  Real.exp (s - a * (i - j : Real))
...
theorem alibi_decay_monotonic 
    (a : Real) (ha : a > 0) 
    (i1 i2 j : Nat) (h_dist : i1 > i2) (hj : i2 >= j) 
    (s : Real) :
  ...
  have h1 : s - a * (i1 - j : Real) < s - a * (i2 - j : Real) := by
    -- Simplify the inequalities
    linarith [
      show (i1 - j : Real) > (i2 - j : Real) by
...
We selected BigScience's \textbf{BLOOM-560m} as our base open-weight model due to its native, hardware-friendly ALiBi attention design. The training dataset consisted of a 4,000,000 character subset of the \textbf{WikiText-2-raw-v1} corpus. 
...
1.  \textbf{Phase 1: Screen}: Each of the 4 hypotheses proposed in Section~\ref{sec:math} was trained on a strict, resource-friendly budget of \textbf{150 steps}.
2.  \textbf{Phase 2: Select}: The best hypotheses from the screening scoreboard were selected for full, deep adaptation runs of \textbf{1200 steps} to confirm if they fully resolved context-scaling and passkey retrieval failures.
...
\begin{itemize}
    \item \textbf{Power Grid Mitigation}: A standard RoPE context-extension pass on Colossus consuming millions of kilowatt-hours can be replaced with our slopes-only $H4$ adaptation, saving \textbf{32.6\% of total electrical energy}.
    \item \textbf{Carbon Footprint Savings}: Scaled across thousands of training nodes, the transition to regularized slopes-only tuning prevents several metric tons of CO${}_2$ from being emitted, aligning corporate AI training with sustainable energy initiatives.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Limited Dataset Scope}: The current evaluation is restricted to the WikiText-2 corpus. Validating generalizability across large multi-domain long-context corpora (such as arXiv, GitHub, or BookCorpus) remains a limitation.
    \item \textbf{Single-Key Retrieval Metrics}: The Passkey task is a single-key associative benchmark. It does not measure multi-hop or multi-key associative memory, which are essential for complex real-world information retrieval.
    \item \textbf{High-Level Energy Estimations}: Carbon and energy metrics are calculated from total training runtimes. They do not subtract system baseline idle power, perform per-kernel hardware profiling (e.g., via NVIDIA DCGM), or incorporate Power Usage Effectiveness (PUE) and regional power grid carbon intensity variations.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Scale-Up and Generalization}: Validate slopes-only adaptation on larger model sizes (e.g., 7B, 13B, and 70B parameters) and across multiple architectures (including bias-swapped RoPE networks) to verify parameter-scaling laws.
    \item \textbf{Extremely Long Context Windows (32k--128k)}: Scale slopes-only adaptation to 32,000 and 128,000 sequence lengths on multi-GPU nodes, evaluating gradient stability and attention-sink behavior.
    \item \textbf{Multi-Key Benchmarking}: Test optimized slopes on complex multi-key retrieval benchmarks (such as RULER's Variable Tracking and Factual Association) to guarantee robust associative retrieval.
    \item \textbf{Direct Baseline Comparison}: Perform detailed, empirical energy-versus-accuracy trade-off evaluations comparing learnable slopes against other techniques, including YaRN interpolation, NTK-Aware scaling, and full parameter fine-tuning.
\end{enumerate}
...
\bibitem{su2023ntk}
B.~Su,
\newblock ``NTK-aware Scaled RoPE,''
\newblock {\em arXiv preprint arXiv:2309.16039}, 2023.
