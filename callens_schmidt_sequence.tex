...
\title{The Callens--Schmidt Sequence $S_{20}(n)$: \\ A New Apéry-like Sequence and its Calabi-Yau 4-fold}
\author{Xavier Callens \\ SocrateAI Laboratory}
\date{\today}
...
In this work, we present the analysis of the case $A=4, B=1$, which we denote as the Callens--Schmidt sequence:
\begin{equation}
S_{20}(n) = \sum_{k=0}^n \binom{n}{k}^4 \binom{n+k}{k}.
\end{equation}
The first few values of $S_{20}(n)$ for $n \ge 0$ are:
\[
1, 3, 55, 1155, 29751, 852753, 26097499, 840454275, \dots
\]
...
\begin{verbatim}
/-- The Callens-Schmidt sequence: S20(n) = Sum_{k=0}^{n} C(n,k)^4 * C(n+k,k) -/
def S20 (n : Nat) : Int :=
