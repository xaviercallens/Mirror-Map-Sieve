...
def fit_recurrence(S, max_d=6, max_m=10):
    n = sp.Symbol('n')
    ...
    for d in range(1, max_d + 1):
        for m in range(1, max_m + 1):
            ...
            if len(S) < num_eqs + d + 20:
                continue
                
            print(f"Trying order d={d}, degree m={m} (vars={num_vars}, eqs={num_eqs})...", flush=True)
            t0 = time.time()
            eqs = []
            ...
            if ns:
                print(f"  Found nullspace size {len(ns)} in {time.time()-t0:.2f}s! Validating...", flush=True)
                for sol in ns:
                    is_valid = True
                    val_start = num_eqs
...
def main():
    ...
    S = [compute_seq(n, 4, 1) for n in range(120)]
    ...
    if res:
        ...
        print("\n❌ Failed to find recurrence relation.")
